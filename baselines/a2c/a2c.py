...
class Runner(object):
    ...
    def __init__(self, env, model, nsteps=5, gamma=0.99, theta=0):
        self.env = env
        self.model = model
        self.replay_buffer = []
    ...
    def run(self):
        ...
        for n in range(self.nsteps):
            ...
            # Save the result of this transition in the buffer
            replay_buffer.append((np.copy(self.obs), self.states, self.dones, actions))
    ...
    def invert_frame(self, frame):
        return (1 - self.theta)*frame + self.theta*(1 - frame)

def learn(policy, env, seed, nsteps=5, total_timesteps=int(80e6), vf_coef=0.5,
          ent_coef=0.01, max_grad_norm=0.5, lr=7e-4, lrschedule='linear',
    ...
    model = Model(policy=policy, ob_space=ob_space, ac_space=ac_space,
                  nenvs=nenvs, nsteps=nsteps, ent_coef=ent_coef,
                  vf_coef=vf_coef, max_grad_norm=max_grad_norm, lr=lr, alpha=alpha,
                  epsilon=epsilon, total_timesteps=total_timesteps,
                  lrschedule=lrschedule, replay_lambda=replay_lambda,
                  ss_rate=ss_rate, replay_loss=replay_loss)
...
def learn_staged(policy, env, seed, nsteps=5, total_timesteps=int(80e6), vf_coef=0.5,
          ent_coef=0.01, max_grad_norm=0.5, lr=7e-4, lrschedule='linear',
          epsilon=1e-5, alpha=0.99, gamma=0.99, log_interval=100,
          replay_lambda=1, ss_rate=1, replay_loss=None, thetas=None):
    tf.reset_default_graph()
    set_global_seeds(seed)
    ...
    for theta in thetas:
        # Reset the runner
        runner = Runner(env, model, nsteps=nsteps, gamma=gamma, theta=theta)
        for update in range(1, total_timesteps//nbatch+1):
            obs, states, rewards, masks, actions, values = runner.run()
            policy_loss, value_loss, policy_entropy = model.train(obs, states, rewards, masks, actions, values)
            nseconds = time.time()-tstart
            ...
            if update % log_interval == 0 or update == 1:
                ...
                logger.record_tabular("explained_variance", float(ev))
                logger.dump_tabular()
        # We're done with the stage, so sample the replay buffer:
        num_samples = int(ss_rate * len(runner.replay_buffer))
        past_data_idx = random.sample(range(0, len(runner.replay_buffer)), num_samples)
        past_data = runner.replay_buffer[past_data_idx]
        model.replay_buffer.extend(past_data)
    env.close()
